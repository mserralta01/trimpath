...
export function AdminAccess() {
  return <>
    <AuthLoading><AdminAuthShell><div className="admin-auth-loading">Checking your Trim Path Rx session…</div></AdminAuthShell></AuthLoading>
...
function AdminGate() {
  ...
  if (!viewer) return <AdminAuthShell><div className="admin-auth-loading">Loading your Trim Path Rx account…</div></AdminAuthShell>;
...
function AdminSignIn() {
  ...
  return <AdminAuthShell><div className="admin-auth-card"><LockKeyhole /><span className="admin-auth-kicker">Independent Trim Path Rx account</span><h1>{flow === "signIn" ? "Sign in to operations." : "Create your Trim Path Rx account."}</h1><p>Manage products, inventory, orders, customers, discounts, certificates, and store settings.</p><form onSubmit={submit}>{flow === "signUp" && <label><span>Name</span><input name="name" autoComplete="name" required /></label>}<label><span>Email address</span><input name="email" type="email" autoComplete="email" required /></label><label><span>Password</span><input name="password" type="password" autoComplete={flow === "signIn" ? "current-password" : "new-password"} minLength={12} required /></label>{flow === "signUp" && <small>Use at least 12 characters with uppercase, lowercase, and a number.</small>}{notice && <div className="admin-auth-error" role="alert">{notice}</div>}<button type="submit" disabled={working}><LockKeyhole />{working ? "Please wait…" : flow === "signIn" ? "Sign in" : "Create account"}</button></form>{(ownerExists === false || flow === "signUp") && <button className="admin-auth-secondary" onClick={() => { setFlow(flow === "signIn" ? "signUp" : "signIn"); setNotice(""); }}>{flow === "signIn" ? "Create the first Trim Path Rx account" : "I already have an account"}</button>}</div></AdminAuthShell>;
}
