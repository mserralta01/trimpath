...
export function AdminDashboard() {
  ...
  return <div className="admin-shell"><aside className={`admin-sidebar ${sidebar ? "is-open" : ""}`}><div className="admin-brand"><Image src="/assets/brand/trimpath-wordmark.svg" width={156} height={38} alt="TrimPath" /><span>OPERATIONS</span><button onClick={() => setSidebar(false)} aria-label="Close navigation"><X /></button></div><nav>{nav.map(([id, label, Icon]) => <button className={view === id ? "is-active" : ""} onClick={() => selectView(id)} key={id}><Icon />{label}<ChevronRight /></button>)}</nav><div className="admin-sidebar__foot"><Link href="/">View storefront</Link><span>Convex operational console</span></div></aside><div className="admin-overlay" onClick={() => setSidebar(false)} />
    <div className="admin-main"><header className="admin-topbar"><div><button className="admin-menu" onClick={() => setSidebar(true)}><Menu /></button><span>{currentLabel}</span></div><label><Search /><input placeholder="Search operations" /></label><div className="environment"><span /> TrimPath Cloud</div><button className="admin-signout" onClick={() => void signOut()}>Sign out</button></header><main className="admin-content">
      {!products?.length && <section className="seed-banner"><div><ClipboardCheck /><span><strong>Initialize the TrimPath catalogue</strong>Seed 12 products, 24 variants, store settings, and a starter discount.</span></div><button onClick={() => seed()}>Seed Convex data</button></section>}
      {view === "overview" && <><div className="admin-heading"><div><span className="admin-kicker">Live operations</span><h1>Good afternoon, Matt.</h1><p>Here is what is happening across TrimPath today.</p></div><button className="admin-primary" onClick={() => setView("orders")}>Review orders</button></div><div className="stat-grid">{[["Gross revenue", money(overview?.revenue || 0), "+12.4%", Activity], ["Orders", String(overview?.orders || 0), "All time", ShoppingCart], ["Customers", String(overview?.customers || 0), "Unique buyers", Users], ["Low stock", String(overview?.lowStock || 0), "Needs attention", PackageSearch]].map(([label, value, hint, Icon]) => <article key={String(label)}><div><span>{label as string}</span><Icon /></div><strong>{value as string}</strong><small>{hint as string}</small></article>)}</div><section className="admin-panel"><div className="panel-heading"><div><h2>Recent orders</h2><p>Latest customer activity across the store.</p></div><button onClick={() => setView("orders")}>View all</button></div><OrderTable orders={overview?.recentOrders || []} onStatus={updateStatus} /></section></>}
