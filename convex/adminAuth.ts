...
export const claimOwner = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (await ctx.db.query("adminUsers").first()) throw new Error("The Axispep owner account has already been created");
    const expected = process.env.AXISPEP_ADMIN_SETUP_CODE;
